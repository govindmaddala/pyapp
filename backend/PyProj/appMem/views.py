...
def getImage(request):
  if request.method == 'POST':
    ...
    #{'names': ['b', 'a'], 'market': ['USA', 'Russia'], 'batch': ['1', '2'], 
    #'productName': ['omez', 'Nise'], 'specificationID': ['s3', 's2']} 

    warnings.filterwarnings('ignore')
    df = pd.DataFrame({'Time Points': [[1], [1, 3, 6], [1, 3, 6, 9], [1, 3, 6, 9, 12], [1, 3, 6, 9, 12, 18], [1], [1, 3],
                                       [1, 3, 6], [1, 3, 6, 9], [1], [1, 3, 6], [
                                           1, 3, 6, 9], [1, 3, 6, 9, 12],
                                       [1, 3, 6, 9, 12, 18], [1], [1, 3], [1, 3, 6], [1, 3, 6, 9]],
                       'Test Results': [[10], [9, 18, 27], [8, 16, 24, 32], [7, 14, 21, 28, 35], [6, 12, 18, 24, 30, 36],
                                        [10], [9, 18], [8, 16, 24], [7, 14, 21, 28], [
                                            10], [9, 18, 27], [8, 16, 24, 32],
                                        [7, 14, 21, 28, 35], [6, 12, 18, 24, 30, 36], [
                                            10], [9, 18], [8, 16, 24],
                                        [7, 14, 21, 28]],
    ...
    # product_names = ['para', 'omez', 'Nice']
    # markets = ['USA', 'Russia', "India"]

    batch_numbers = data['batch']
    names = data['names']
    specification_ids = data['specificationID']
    product_names = data['productName']
    markets = data['market']
    df1 = fill_missing_values(df, batch_numbers, names,
                              specification_ids, product_names, markets)
    ...
    df1['New_Test_Results'] = df1['New_Test_Results'].apply(
        lambda x: [0 if pd.isna(i) else i for i in x])
    df1['New_Time_Points'] = df1['New_Time_Points'].apply(
        lambda x: [0 if pd.isna(i) else i for i in x])
    ...
    def plot_results(df):
        ...
        for idx, val in df1.iterrows():
            ...
            fig.add_trace(go.Scatter(x=x, y=y_2, mode='lines+markers', name=b_no, legendgroup=b_no, marker=dict(color=color),
            ...
            fig.add_trace(go.Scatter(x=x_pred, y=y_pred, mode='lines+markers', name=b_no, line=dict(dash='dot', color=color),
        ...
        return fig.show()
    return HttpResponse(plot_results(df1))
